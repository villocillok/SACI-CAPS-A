        ...
        <div class="row" style="height: 100%;">
            <div id="cell-sidebar" class="cell bg-lightDark size-x300" style="overflow-y: scroll; height: 100%;">
                <ul class="sidebar2 dark">
                    ...
                    <li class="divider"></li><li class="divider"></li>
                    <li class="active"><a href="./manage_faculties.php"><span class="mif-users icon"></span><span class="title">Librarian Information</span></a></li>
            ...
            <div id="cell-content" class="cell auto-size padding20 pane-scroll">
                <h1 class="text-light">Manage Librarian Information</h1>
                <hr style="background-color:#493867;"><br>
                <div class="align-right">
                    <a class="button primary beautify beautify-mif" data-button="add-faculty-button" data-role="hint" data-hint="Add Account" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-plus"></span>&nbsp;&nbsp;Add</a>
                </div>
                <table id="faculties-table" class="table striped border bordered">
                    <thead>
                        <tr>
                            <th width="15%">Librarian ID</th>
                            <th>Name</th>
                            <th>Books(s) On Hand</th>
                            <th>Type</th>
                            <th width="15%"></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                            ...
                            $connection->query("SELECT * FROM accounts INNER JOIN librarian ON accounts.Account_ID=librarian.Librarian_ID");

                            while($row = $connection->fetch_assoc()) {
                                if(strlen($row['Librarian_Middle_Name']) > 1) {
                                    $name = $row['Librarian_Last_Name'] . ', ' . $row['Librarian_First_Name'] . ' ' . substr($row['Librarian_Middle_Name'], 0, 1) . '.';
                                } else {
                                    $name = $row['Librarian_Last_Name'] . ', ' . $row['Librarian_First_Name'];
                                }

                                echo '<tr>';
                                echo '<td class="align-center">' . $row['Librarian_ID'] . '</td>';
                                echo '<td>' . $name . '</td>';
                                echo '<td>' . $row['On_Hand'] . '</td>';
                                echo '<td>' . $row['Account_Type'] . '</td>';
                                echo '<td class="align-center"><a class="button success mini-button beautify" data-button="edit-faculty-button" data-var-id="' . $row['Librarian_ID'] . '" data-role="hint" data-hint="Edit Account" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-pencil"></span></a><a class="button danger mini-button beautify" data-button="delete-faculty-button" data-var-id="' . $row['Librarian_ID'] . '" data-role="hint" data-hint="Delete Account" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-cross"></span></a></td>';
                                echo '</tr>';
                            }
...
<script src="assets/js/manage_faculties.js"></script>
