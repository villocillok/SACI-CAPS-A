        ...
        <div class="row" style="height: 100%;">
            <div id="cell-sidebar" class="cell bg-lightDark size-x300" style="overflow-y: scroll; height: 100%;">
                <ul class="sidebar2 dark">
                    ...
                    <li class="active"><a href="./manage_students.php"><span class="mif-users icon"></span><span class="title">Borrower Information</span></a></li>
            ...
            <div id="cell-content" class="cell auto-size padding30 pane-scroll">
                <h1 class="text-light">Manage Borrower Information</h1>
                <hr style="background-color:#493867;"><br>
                <div class="align-right">
                    <a class="button primary beautify beautify-mif" data-button="add-student-button" data-role="hint" data-hint="Add Borrower" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-plus"></span>&nbsp;&nbsp;Add</a>
                </div>
                <table id="students-table" class="table striped border bordered">
                    <thead>
                        <tr>
                            <th width="15%">Borrower ID</th>
                            <th>Name</th>
                    ...
                    <tbody>
                        <?php
                            ...
                            $connection->query("SELECT * FROM accounts INNER JOIN borrower ON accounts.Account_ID=borrower.Borrower_ID");

                            $connection1->query("SELECT * FROM borrower INNER JOIN department ON borrower.Department_ID=department.Department_ID");


                            while($row = $connection->fetch_assoc()) {
                                if(strlen($row['Borrower_Middle_Name']) > 1) {
                                    $name = $row['Borrower_Last_Name'] . ', ' . $row['Borrower_First_Name'] . ' ' . substr($row['Borrower_Middle_Name'], 0, 1) . '.';
                                } else {
                                    $name = $row['Borrower_Last_Name'] . ', ' . $row['Borrower_First_Name'];
                                }
                                ...
                                echo '<td class="align-center"><a class="button success mini-button beautify" data-button="edit-student-button" data-var-id="' . $row['Borrower_ID'] . '" data-role="hint" data-hint="Edit Borrower" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-pencil"></span></a><a class="button danger mini-button beautify" data-button="delete-student-button" data-var-id="' . $row['Borrower_ID'] . '" data-role="hint" data-hint="Delete Borrower" data-hint-background="#dc4fad" data-hint-color="#ffffff" data-hint-mode="2" data-hint-position="top"><span class="mif mif-cross"></span></a></td>';
                                echo '</tr>';
                            }
